...
import com.example.usuariocliente.Cliente.Home.MiRentaSelected;
...
public class MisRentasAdapter extends RecyclerView.Adapter<MisRentasAdapter.RentaViewHolder> {
    List<Renta> rentas;
    ArrayList<Renta> arrayList;
    Context c;

    public MisRentasAdapter(List<Renta> rentas) {
        this.rentas = rentas;
    ...
    @NonNull
    @Override
    public MisRentasAdapter.RentaViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.my_rent_card, parent, false);
        MisRentasAdapter.RentaViewHolder rvh = new MisRentasAdapter.RentaViewHolder(v);
        c = v.getContext();
        return rvh;
    }
    ...
    public void onBindViewHolder(@NonNull RentaViewHolder holder, int position) {
        ...
        if (rentas.get(position).getStatus() == 1)
            holder.entregar.setText("Entregar");
        holder.cv.setOnClickListener(v -> {
            Renta renta = rentas.get(position);
            //Toast.makeText(c, renta.getId_usuario()+"", Toast.LENGTH_SHORT).show();
            Intent i = new Intent(c, MiRentaSelected.class);
